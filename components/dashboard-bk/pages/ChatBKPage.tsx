              ...
              <div className="w-full">
                ...
                {messages.length > 0 ? (
                  messages.map((message: Message, index: number) => {
                    ...
                    const topicDisplay = currentReservasi?.topic ? `📚 Sesi chat: ${currentReservasi.topic?.name || 'Konseling'}${currentReservasi.notes ? ` | ${currentReservasi.notes}` : ''}` : ((activeCounselor as FormattedConversation)?.topic ? `📚 Sesi chat: ${(activeCounselor as FormattedConversation).topic}` : null)
