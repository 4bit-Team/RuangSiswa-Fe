...
const HistoryItem: React.FC<HistoryItemProps> = ({ title, counselor, date, status, statusColor }) => (
  <div className="flex items-center justify-between p-4 border-b border-gray-200">
    ...
    <span
      className={`px-3 py-1 rounded-full text-xs font-medium ${statusColor} bg-opacity-10`}
    >
      {status}
    </span>
  </div>
...
const ProfilPage: React.FC = () => {
  ...
  // Ambil data user dari cookie auth_profile
  useEffect(() => {
    try {
        ...
        <div className="flex items-center gap-6 mb-6">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-900">
              {user?.username || 'Nama belum diatur'}
            </h2>
            <p className="text-gray-500">
              {user?.kelas ? `Kelas ${user.kelas.nama}` : 'Kelas belum diatur'}
            </p>
            <p className="text-gray-500">
              {user?.jurusan ? user.jurusan.nama : 'Jurusan belum diatur'}
            </p>
